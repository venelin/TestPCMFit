...
tableFits1 <- rbindlist(lapply(list.files(".", pattern=paste0("fits_Mammals2_t1_*")), function(f)  {
  load(f)
  data
}))
...
tableFits6 <- rbindlist(lapply(list.files(".", pattern=paste0("fits_Mammals_t6_*")), function(f)  {
  load(f)
  data
}))

tableFits <- rbindlist(list(tableFits, tableFits1, tableFits2, tableFits4, tableFits5, tableFits6))
...
fitMappings <- PCMFitModelMappings(
  ...
  prefixFiles = paste0("fits_Mammals2_t2_"),
...
save(fitMappings, file = paste0("fitMappings_Mammals2_t2_.RData"))
