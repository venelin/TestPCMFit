...
#load("../local-data/ResultsMammalData/FinalResult_MixedGaussian_MammalData_id_1_t1_.RData")
#load("../local-data/ResultsMammalData/FinalResult_MixedGaussian_MammalData_id_1_t3_.RData")
#load("../local-data/ResultsMammalData/FinalResult_MixedGaussian_MammalData_id_1_t5_.RData")

# t6 corresponds to the fit with models A-F
load("../local-data/ResultsMammalData/FinalResult_MixedGaussian_MammalData_id_1_t6_.RData")

#load("../local-data/ResultsMammalData/FinalResult_MixedGaussian_MammalData_id_1_t7_.RData")
#load("../local-data/ResultsMammalData/FinalResult_ScalarOU_MammalData_id_1_t8_.RData")
#
# The "Result_FineTuning_BestFit_MammalData_t1.RData file contains the refined fit from t6 (improving the
# AIC from -237 to -241, by iteratively fixing all but one regimes, while optimizing the left free regime
# with respect to model type and parameters). This step was performed in a separate R-script 
# TuneBestFitMammalData_t6.R.
load("../local-data/ResultsMammalData/Result_FineTuning_BestFit_MammalData_t6.RData")
...
dtCladePartition[node %in% PCMTreeGetStartingNodesRegimes(inferredTree), selected:=TRUE]
dtCladePartition[list(PCMTreeGetStartingNodesRegimes(inferredTree)), 
...
```{r analyze-simulations-t1, include=FALSE, eval=FALSE}
testResultData <- rbindlist(
  lapply(1:nrow(testData), function(i) {
    resultFile <- paste0("../local-data/ResultsTestData_t1/FinalResult_MixedGaussian_testData_id_", i, "_t1_.RData")

    if(file.exists(resultFile)) {
      cat("Loading ", resultFile, "\n")
      load(resultFile)
      bestFitAIC <- RetrieveBestFitAIC(fitMappings)
      trueFromTestData <- RetrieveTrueFromTestData(testData, i)
...
```{r fig7-plot-sim-results-t1, fig.width=8, fig.height=6.66, eval=evalFig7.1}
load("../local-data/ResultsTestData_t2/TestResultData_t1.RData")
...
if(file.exists("../local-data/ResultsTestData_t2/TestResultData_t2.RData") ) {
  load("../local-data/ResultsTestData_t2/TestResultData_t2.RData")
} else {
  testResultData <- NULL
}
testResultDataNew <- rbindlist(
  lapply(1:nrow(testData_t2), function(i) {
    ...
    if(is.null(testResultData) || !(i %in% testResultData[, unique(i)]) ) {
      resultFile <- paste0("../local-data/ResultsTestData_t2/FinalResult_MixedGaussian_testData_t2_id_", i, "_t2_.RData")
...
save(testResultData, file="../local-data/ResultsTestData_t2/TestResultData_t2.RData")
```
...
```{r fig7-scatter-plots-simulations-t3, fig.width=9, fig.height=9, eval=evalFig7.4}
data <- rbindlist(lapply(1:nrow(testData_t3), function(i) {
  pl <- PCMPlotTraitData2D(
    X = testData_t3$X[[i]][, 1:PCMTreeNumTips(testData_t3$treeWithRegimes[[i]])],
    tree = testData_t3$treeWithRegimes[[i]])
  cbind(pl$data, testData_t3[i, list(rowId = c(i, rep(as.integer(NA), nrow(pl$data)-1)), treeType, treeSize, numClusters, clusterNodes, mapping, logLik, AIC, nobs, df, IdMappingForClustering, IdParamForMapping, IdSimulationForParam)])
}))
...
ggplot(data[treeType == "ultrametric" & treeSize=="small" & numClusters==2], aes(x, y)) + 
  geom_point(aes(color = regime), size = .05, alpha = .5) +
  geom_point(x=-4, y=-5) +
  scale_x_continuous(limits = xlim) +
  scale_y_continuous(limits = ylim) +
  geom_label(aes(x=-12, y=12, label = rowId)) +
  facet_grid(IdParamForMapping+IdSimulationForParam~IdMappingLETTERS) +
  theme_bw()
ggplot(data[treeType == "ultrametric" & treeSize=="small" & numClusters==8], aes(x, y)) + 
  geom_point(aes(color = regime), size = .05, alpha = .5) +
  geom_point(x=-4, y=-5) +
...
# Randomization test via shuffling data within regimes
We are interested in whether the best fit to the mammal data has been informed from the phylogenetic  relationship (phenotype differences with respect to time) between the tips in each regime. If we shuffle the tip-values of all tips within a regime, that destroys any correlation between the differences in trait values in couples of tips with respect to their phylogenetic distance. At the same time, the marginal distribution of these tips (i.e. the body-brain-mass scatter plots) within the regimes will remain exactly the same. If the shuffling does not affect the AIC score significantly, this would imply that the fit ignores the phylogenetic correlation.
```{r shuffle-clusters, eval=FALSE}
tree <- attr(inferredModel, "tree")
X <- attr(inferredModel, "X")

AIC0 <- AIC(inferredModel)

AIC_s <- sapply(1:100, function(n) {
  AIC_si <- rep(0.0, PCMTreeNumUniqueRegimes(tree))
  names(AIC_si) <- as.character(PCMTreeUniqueRegimes(tree))
  for(i in PCMTreeUniqueRegimes(tree)) {
    tips <- PCMTreeGetTipsInRegime(tree, i)
    X_si <- X
    X_si[, tips] <- X_si[, sample(tips)]
    inferredModel_si <- inferredModel
    attr(inferredModel_si, "X") <- X_si
    AIC_si[i] <- AIC(inferredModel_si)
  }  
  AIC_si
})
